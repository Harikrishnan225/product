...
import { updateSupplier } from "../../services/Supplier/Supplier";
import { useParams } from "react-router-dom";

function SalesLedgerEdit() {
      ...
      try {
        ...
        if (!isNaN(numericId)) {
          const convertAllToNumber = () => {
            const data = Object.entries(salesLedger).map(([k, v]) => [k, +v]);
            return Object.fromEntries(data);
          };
          await updateSupplier(numericId, convertAllToNumber());
        }
      } catch (error) {
        console.log("Sales Ledger Edit", error);
      }
    }
  };
  return (
...
export default SalesLedgerEdit;
